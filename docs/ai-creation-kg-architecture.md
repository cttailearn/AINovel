...
> 实施状态 (2026-06): **①②③ 全部已落地, 见代码**:
> - ① 抽取差异可追溯: migration 已落 (ai_chapter_variants / ai_chapters 都加了 kg_extracted_at / kg_entity_count / kg_event_count), 见 `backend/database.py::_migrate_to_v1_schema`
> - ② Critic kg_conflicts 接入: 已在 `services/creation_agents.py` 的 critic prompt 中要求 `conflicts_in_text` 字段, `reconcile_kg_conflicts` 已在 `db/ai_kg.py`, `update_ai_chapter_compass` 在 `db/ai_kg.py`
> - ③ KG 上下文主动降噪: 已实现 `db.kg.kg_system_from_request` 等统一读层, `KgSystem` 枚举 + `unified_graph_shape` 提供稳定的"返回 top N 实体"接口; RAG 排序在 `services/kg_service.py` 内有相关性打分
> - 统一读视图: `db/kg.py` 提供 `KgSystem.AI / NOVEL` 枚举 + `DEPRECATED_NOTICE`, `?kg_system=ai|novel` 已在 `/api/novels/{id}/knowledge-graph` 路由透出 (见 `backend/routers/novels.py`)
